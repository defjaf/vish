...
double p_proj(double kappa)
{
    ...
    return prefac_loc*qgaus_ahj(Pintegrand, 0.0, 1.0);
#else 
    static double prefac_loc=1./(16.0*PI*PI);
    kappa_g=kappa;
    return prefac_loc*qgaus_ahj(Pintegrand_1, 0.0, 1.0);
...
double xintegrand_old(double x)
{
    ...
    return P(kp*sqrt(1.+sqr(dummyy)-2.*x*dummyy)) 
        * (1.-x*x) * sqr(1.-2.*x*dummyy) 
        / (1.+sqr(dummyy)-2.*x*dummyy); 
}
