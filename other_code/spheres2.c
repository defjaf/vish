/* From: Marc Kamionkowski <kamion@astro.columbia.edu> */
/* To: jaffe@cfpa.berkeley.edu */
/* Subject: spheres.c */
/* Date: Thu, 21 May 1998 18:07:43 -0400 (EDT) */
...
#include "vish.h"
#include "/home/carmen/kamion/recipes_c/nrutil.c"
#include "/home/carmen/kamion/recipes_c/qromb.c"
#include "/home/carmen/kamion/recipes_c/qgaus.c"
#include "/home/carmen/kamion/recipes_c/trapzd.c"
#include "/home/carmen/kamion/recipes_c/polint.c"
#include "/home/carmen/kamion/recipes_c/splint.c"
#include "/home/carmen/kamion/recipes_c/spline.c"
...
static double dummyk,Radius,ell,k_max;
...
main(int argc,char *argv[])
{
  ...
  */
  ...
  spline(wz_array,w_array,1100,1.e40,1.e40,w_2);
  spline(w_array,wz_array,1100,1.e40,1.e40,z_2);
  ...
  */
  ...
  /*  print Cl_integrand as diagnostic
  ell=pow(10.,3.32);
  for(w=w_low;w<=w_high;w+=(w_high-w_low)/1000.) { 
    printf("%e  %e\n",w,Cl_integrand(w)); 
  } 
  exit(1);
  */
  ...
  /*  first we find the maximum ell for which Cl_integrand isn't zero  */
  if(fabs(1.-Omega-Omega_vac)<SMALLNUM) {
    RS_chi_max=w_high;
  }
  else {
    if(1.-Omega-Omega_vac>0.) {
      RS_chi_max=0.5/sqrt(1.-Omega-Omega_vac) 
	       * sinh(2.0*sqrt(1.-Omega-Omega_vac)*w_high);
    }
    else {
      RS_chi_max=0.5/sqrt(-1.+Omega+Omega_vac) 
	       * sin(2.0*sqrt(-1.+Omega+Omega_vac)*w_high);
    }
  }
  patchyfile=fopen("patchy.out","w");
  for(logell=1.;logell<=log10(k_max*RS_chi_max);logell+=0.1) {
    ell=pow(10.,logell);
	...
	*Cl_integrand(w_i)*(w_high-w_low);
    ...
    fprintf(patchyfile,"%e  %e  %e  %e  %e  %e  %e\n",
	    ell,sqr(ell)*Cl/2./PI,sqr(ell)*Cl_approx/2./PI,sqr(ell)*ClGH/2./PI,
	    sqr(ell)*ClGH/2./PI*GH/numerical,GH,numerical);
    /*  printf("%e  %e  %e\n",ell,numerical,GH);  */
  }
  fclose(patchyfile);
}
