/*    calculate Cl's for vishniac effect in flat Universe for given
      ...
      x_e = |x0 (1+z)^b = x0 (a/a0)^-b = x0 (eta/eta0)^-2b  eta_m<eta<eta_1
            |0                                              eta < eta_m
      (need params x0, xbar, xm, b)

      nb. program only needs one of x_e, tau_r, z_r (=zzz(eta0-eta_m))

      2. arbitrary P(k), read in from a file
...
#define SMALLNUM (1.e-30)   /* for ==0.0 checks */
...
static int do_XeXe_corr, do_Xed_corr;
...
int main(int argc, char *argv[])
{
    ...
    printf("\nGaussian visibility function [>0 = yes]? ");
    ...
    if (do_Gauss_vis<1) {
        ...
        for(z=0.;z<=1100.;z+=1.) {
#if defined(USE_LNG)
            fprintf(zoutfile,"%f %f %f  %g\n",z,www(z),tau_prefactor*tau(z),
...
double x_e(double z)
{
    ...
    if (do_XeXe_corr || do_Xed_corr) { /* patchy */
...
/* the visibility function, g(eta) = tau' e^-tau
   given the model 
 *    x_e = x_0 (1+z)^alpha = x_0 (eta/eta0)^-2alpha;   eta_m < eta < eta_1
 */
...
double p_proj(double kappa)
{
    ...
    if (first && (do_XeXe_corr || do_Xed_corr)) {
        /* only integrate over time when ionization "turns on" */
        ymin=www(z_i-delta_zi/2.0)/ETA0;
        ymax=www(z_i+delta_zi/2.0)/ETA0;
	printf("\nymin, ymax=%f %f\n", ymin, ymax);
        first=0;
    }
    kappa_g=kappa;    
